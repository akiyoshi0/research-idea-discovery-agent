...
- `decision_log.md`と`research_plan.md`は、論文の表現を貼り合わせるのではなく、研究者が研究計画として読める日本語で書く。
- 論文由来の英語専門語をそのまま並べない。初出では必ず短い日本語説明を添える。
- データセット名、モデル名、正式な評価指標名は原表記を残してよいが、概念語はできるだけ日本語で噛み砕く。
- 「示唆される」「期待される」「未整理である」を多用せず、「何を検証するか」「なぜ重要か」「どう失敗しうるか」を直接書く。
