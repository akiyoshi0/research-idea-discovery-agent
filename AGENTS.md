...
- probeを最終実験として扱わない。
- 必要な依存ライブラリや外部toolが未導入の場合、勝手に別ライブラリ・内製実装・簡易fallbackを追加しない。失敗理由を記録し、依存導入、手動作成、または明示的な代替案承認を人間に確認する。
...
ユーザーがskill名だけを指定し、具体的な作業対象や依頼内容がない場合は、通常の「何を確認したか」形式で長く説明しない。
